...
## Inventory refresh — September 12, 2026

Backed up private SQLite/environment state and retained a private baseline before collection. Refreshed enabled regional catalogs with `--fresh --pages=500 --details=50`, completed Volo's one remaining detail with a zero-page follow-up, then refreshed the distinct national ClassicCars catalog with `--fresh --pages=500 --details=0`. Existing spacing, source health, permissions and budgets remained in force; no worker or external delivery was started.

The run fetched 94 fresh catalog pages, added 79 ads, refreshed 226 details and observed 47 numeric asking-price changes. One Volo ad newly reported sold. Two J & S Motors detail URLs returned 404 and retained their prior observations; Autotrader and 500 Classic remained paused without probes. Five dealer detail queues completed, while ClassicCars retains 3,026 due details after its bounded 50-detail batch. No missing ad was inferred sold or removed.

Exported 3,257 public ads in 3,241 public groups with 33 detail chunks, rebuilt both static paths, and passed the dated privacy/consistency checks. A second consistent private backup preserves the result. Current README, handoff, source coverage, validation and remaining-work documentation link the new [comparison](docs/validation/refresh-2026-09-12-comparison.md); earlier evidence remains dated. This refresh changes data and documentation only.

Committed and pushed the inventory refresh as `b4e0914`. The [Pages workflow](https://github.com/PNelsonFTP/ClassicCars/actions/runs/34694382537) completed successfully, and [live verification](docs/validation/github-pages-2026-09-12.json) confirmed the new snapshot and responsive behavior. The closing documentation commit records deployment evidence only. Collection is stopped and the automatic worker remains off.
