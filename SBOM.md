...
September 12 source-access update: no dependency or lockfile changes. The diagnostic browser uses the existing development Playwright package; the normal collector and GitHub Pages do not require a browser runtime. SBOM provenance is regenerated because package.json now includes diagnostic commands; dependency versions and the lockfile are unchanged.
...
The September 10 GitHub matrix now captures and validates native/SBOM evidence for Node 24 on Ubuntu, Windows and macOS, Node 22.18.0 on Ubuntu, and the Linux container. The dated [publication evidence](docs/DELIVERY.md) records the target artifacts separately from the September 8 reports above. Other unexecuted architectures remain unverified. Container dpkg evidence inventories observed Debian packages; manually installed OS files, the host operating system, downloaded browsers, complete embedded native component graphs and GitHub runner images remain outside the main npm SBOM. Action commits and the multi-platform Node base digest are pinned; apt and hosted runner inputs are still time-dependent.

The [September 10 target receipt](docs/validation/github-release-2026-09-10.json) preserves exact inputs, counts and artifact hashes. Full dependency counts (excluding the application) are 367 for Node 24 Linux, 369 for Node 22 Linux, 368 for macOS, 366 for Windows and 367 for the Linux x64 container. Native/WASM file counts are 30, 31, 31, 30 and 30 respectively. The Ubuntu runner's 1,211 dpkg packages describe the hosted runner; the container's 91 packages describe the built image. GitHub artifact retention is 14 days; complete downloaded reports are retained in the private local release archive named in the delivery record. Windows package/lock byte hashes differ because of Git CRLF checkout, while the parsed lock dependency inventory is unchanged.
...
The September 10 publication added conservative npm 10 identity normalization. Repeated package/version IDs are coalesced only when all non-location metadata agrees. CycloneDX retains every install-path property and dependency edge; SPDX retains all locations in an annotation and all distinct relationships. Conflicting metadata, duplicate final IDs and unresolved references fail validation. Eleven regression tests and exact Node 22.18.0/npm 10.9.3 checks cover this behavior. Vendored schemas have a Git `-text` attribute to preserve their pinned bytes on Windows; hash validation is unchanged. These tooling fixes do not alter the lockfile or redate the September 8 advisory report.
