...
| P2-10 | Clean Node 24 macOS and Linux Docker builds; isolated setup/migration/SQLite/restore/startup checks; CI platform matrix | Execute Windows launcher and remote CI; Docker Compose multi-service deployment remains a separate environment check. |
| P2-11 | Native Chromium WebMCP registration/lifecycle checks and root/subpath static export validation | Actual authorized Pages/HTTPS-to-private-network behavior needs a destination. |
...
| P2-13 | Full offline SBOM schema/hash validation, host/container native+OS inventories and exact optional-WASM manifest inspection | Windows target artifacts and deeper embedded native/Rust component inventories remain target-specific work. |
...
The release workflow needs its first real GitHub run, including Windows. Local Linux arm64 Docker and clean macOS Node 24 checks passed; they do not prove Windows or every architecture. The Docker static UI contains build-time JSON and needs an explicit refresh/rebuild or connected mode. OS autostart remains optional and was not installed.
